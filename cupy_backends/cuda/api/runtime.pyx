...
IF CUPY_USE_GEN_HIP_CODE:
    from cupy_backends.cuda.api.runtime_hip import *
ELSE:
    ###########################################################################
    # Classes
    ###########################################################################
    ...
    cdef class MemPoolProps:
        ...
        def __init__(self, int allocType, int handleType,
                     int locationType, int devId):
    ...
    ELSE:
        IF CUPY_HIP_VERSION > 0:
            include '_runtime_extern_hip.pxi'
        ELSE:
    ...
    cpdef int runtimeGetVersion() except? -1:
        cdef int version
        IF CUPY_USE_CUDA_PYTHON:
            # Workarounds an issue that cuda-python returns its version instead
            # of the real runtime version.
    ...
    cpdef str deviceGetPCIBusId(int device):
        # The PCI Bus ID string must be able to store 13 characters including
        # the NULL-terminator according to the CUDA documentation.
    ...
    cpdef intptr_t malloc3DArray(intptr_t descPtr, size_t width, size_t height,
        ...
        with nogil:
            status = cudaMalloc3DArray(&ptr, <ChannelFormatDesc*>descPtr,
                                       extent, flags)
        check_status(status)
        return <intptr_t>ptr
    ...
    cpdef intptr_t mallocAsync(size_t size, intptr_t stream) except? 0:
        ...
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('mallocAsync requires ROCm 5.2+')
    ...
    cpdef intptr_t mallocFromPoolAsync(
        ...
        cdef void* ptr
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('mallocFromPoolAsync require ROCm 5.2+')
        if runtimeGetVersion() < 11020:
            raise RuntimeError('mallocFromPoolAsync is supported '
                               'since CUDA 11.2')
        with nogil:
    ...
    cpdef freeAsync(intptr_t ptr, intptr_t stream):
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('freeAsync requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
    ...
    cpdef memcpyPeer(intptr_t dst, int dstDevice, intptr_t src, int srcDevice,
                     size_t size):
        with nogil:
            status = cudaMemcpyPeer(<void*>dst, dstDevice, <void*>src,
                                    srcDevice, size)
        check_status(status)

    cpdef memcpyPeerAsync(intptr_t dst, int dstDevice, intptr_t src,
                          int srcDevice, size_t size, intptr_t stream):
        with nogil:
    ...
    cpdef memcpy2D(intptr_t dst, size_t dpitch, intptr_t src, size_t spitch,
        ...
        with nogil:
            status = cudaMemcpy2D(<void*>dst, dpitch, <void*>src, spitch,
                                  width, height, kind)
        check_status(status)
    ...
    cpdef memcpy2DFromArray(intptr_t dst, size_t dpitch, intptr_t src,
        ...
        with nogil:
            status = cudaMemcpy2DFromArray(<void*>dst, dpitch, <Array>src,
    ...
    cpdef memcpy2DToArray(intptr_t dst, size_t wOffset, size_t hOffset,
                          ...
                          size_t height, int kind):
        with nogil:
            status = cudaMemcpy2DToArray(<Array>dst, wOffset, hOffset,
    ...
    cpdef intptr_t deviceGetDefaultMemPool(int device) except? 0:
        ...
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('deviceGetDefaultMemPool requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
    ...
    cpdef intptr_t deviceGetMemPool(int device) except? 0:
        ...
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('deviceGetMemPool requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
    ...
    cpdef deviceSetMemPool(int device, intptr_t pool):
        ...
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('deviceSetMemPool requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
    ...
    cpdef intptr_t memPoolCreate(MemPoolProps props) except? 0:
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('memPoolCreate requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
    ...
    cpdef memPoolDestroy(intptr_t pool):
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('memPoolDestroy requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
    ...
    cpdef memPoolTrimTo(intptr_t pool, size_t size):
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('memPoolTrimTo requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
    ...
    cpdef memPoolGetAttribute(intptr_t pool, int attr):
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('memPoolGetAttribute requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
            raise RuntimeError('memPoolGetAttribute is supported '
                               'since CUDA 11.2')
        cdef int val1
        cdef uint64_t val2
        cdef void* out
        ...
        with nogil:
            status = cudaMemPoolGetAttribute(<MemPool>pool, <MemPoolAttr>attr,
                                             out)
        check_status(status)
        # TODO(leofang): check this hack when more cudaMemPoolAttr are added!
    ...
    cpdef memPoolSetAttribute(intptr_t pool, int attr, object value):
        if 0 < CUPY_HIP_VERSION < 50200000:
            raise RuntimeError('memPoolSetAttribute requires ROCm 5.2+')
        if runtimeGetVersion() < 11020:
            raise RuntimeError('memPoolSetAttribute is supported '
                               'since CUDA 11.2')
        cdef int val1
        cdef uint64_t val2
        cdef void* out
        ...
        if attr <= 0x3:
            ...
            out = <void*>(&val2)
        with nogil:
            status = cudaMemPoolSetAttribute(<MemPool>pool, <MemPoolAttr>attr,
                                             out)
        check_status(status)
    ...
    cpdef float eventElapsedTime(intptr_t start, intptr_t end) except? 0:
        cdef float ms
        status = cudaEventElapsedTime(&ms, <driver.Event>start,
                                      <driver.Event>end)
        check_status(status)
        return ms
    ...
    cdef ResourceDesc getTextureObjectResourceDesc(uintmax_t obj) except*:
        ...
        with nogil:
            status = cudaGetTextureObjectResourceDesc(&desc,
                                                      <TextureObject>obj)
        check_status(status)
        return desc
    ...
    cpdef graphLaunch(intptr_t graphExec, intptr_t stream):
        with nogil:
            status = cudaGraphLaunch(<GraphExec>(graphExec),
                                     <driver.Stream>stream)
        check_status(status)
    ...
    cpdef graphUpload(intptr_t graphExec, intptr_t stream):
        ...
        with nogil:
            status = cudaGraphUpload(<GraphExec>(graphExec),
                                     <driver.Stream>stream)
        check_status(status)
